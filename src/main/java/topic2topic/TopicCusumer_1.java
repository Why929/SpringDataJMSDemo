...
/**
 * 需要接收方先开启:然后,在开启发送方发送,不然启动不了
 */
public class TopicCusumer_1 {
...
//        Queue queue = session.createQueue("test-queue");
        Topic topic_groupName_B = session.createTopic("pinyougou_topic_page_solr");
...
//                8.转换坚挺到的 消息类型:五大消息类型之一
                ...
                try {
                    System.out.println("Cusumer_1 收到消息->"+textMessage.getText());
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
